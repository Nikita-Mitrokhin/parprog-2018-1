...
public:   enum ext_cls { NO = 1, VERDICT, MESSAGE, TIME, MEMORY };       result(bool read = false)
{
	if (read) fopen_s(&bur, "..//result.txt", "r");
		  ...
		  // Сообщить тестирующей системе ускорение работы программы участника,
		   void write_boost(long long x)
		  {
			  write_type(ext_cls::TIME); fwrite(&x, sizeof(x), 1, bur);
		  }
} checker_result; //////////////////////////////////////////////////////////////////////////////////////////// 
...
int main(int argc, char* argv[])
{
	...
	fread(&Boost, sizeof(Boost), 1, stdin);
	...
	checker_result.write_boost(Boost);
	fclose(f1);
